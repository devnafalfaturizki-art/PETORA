...
export const Table: Component<TableProps> = (props) => {
  ...
  return (
    <div class="w-full overflow-x-auto rounded-lg border border-border bg-background">
      <table
        class={`w-full border-collapse text-left text-sm ${local.class ?? ""}`}
      >
        <thead class="bg-muted/50">
          <tr>
            <For each={local.headers}>
              {(header) => (
                <th
                  class={`px-4 py-2.5 font-medium text-muted-foreground ${header.class ?? ""}`}
                >
                  {header.label}
                </th>
              )}
...
export const DataTable = <T extends object>(props: DataTableProps<T>) => {
  ...
  const resolvedLoading = createMemo(() =>
    typeof local.isLoading === "function"
      ? local.isLoading()
      : !!local.isLoading,
  );
  ...
  return (
    <div
      class={`w-full overflow-x-auto rounded-lg border border-border bg-background ${local.class ?? ""}`}
      {...others}
    >
                  ...
                  {(row) => (
                    ...
                    >
                      <For each={local.columns}>
                        {(column) => (
                          <td
                            class={`px-4 py-2.5 align-middle ${column.cellClass ?? ""}`}
                          >
