import { type Component, splitProps } from 'solid-js';
import type { LoadingSpinnerProps } from '@/components/ui/types';

export const LoadingSpinner: Component<LoadingSpinnerProps> = (props) => {
  const [local] = splitProps(props, ['size', 'label', 'class']);
  const sizeClass = {
    sm: 'h-4 w-4',
    md: 'h-6 w-6',
    lg: 'h-10 w-10',
  }[local.size ?? 'md'];

  return (
    <div class={local.class ?? 'flex items-center justify-center'}>
      {local.label ? (
        <div class="flex flex-col items-center gap-2">
          <svg
            class={`animate-spin ${sizeClass} text-muted-foreground`}
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
          >
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.125 5.824 3 7.938l3-2.647z" />
          </svg>
          <span class="text-sm text-muted-foreground">{local.label}</span>
        </div>
      ) : (
        <svg
          class={`animate-spin ${sizeClass} text-muted-foreground`}
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
        >
          <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
          <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.125 5.824 3 7.938l3-2.647z" />
        </svg>
      )}
    </div>
  );
};

export default LoadingSpinner;
